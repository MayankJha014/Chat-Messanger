...
const Dailog = ({ open, setOpen, children, width, dialogText }) => {
  return (
    <>
      <div
        className={`bg-black/20 w-screen h-screen backdrop-blur-sm fixed top-1/2 left-1/2 z-[40] -translate-x-2/4 -translate-y-1/2 ${
          open ? "block" : "hidden"
        }`}
        onClick={() => {
          setOpen(!open);
      ...
      >
        <div className="flex justify-between items-center rounded-lg bg-gray-200 py-2 m-0 ">
          <div className=""></div>
          <h1 className="font-robot font-bold">{dialogText}</h1>
          <GrFormClose
            size={30}
